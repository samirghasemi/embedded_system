 ...
 * Component: Chart1
 * Component Simulink Path: Chart1
 * Simulation Mode: PIL
 *
 */

#define S_FUNCTION_NAME                Chart1_pbs
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("p2_1");
      mxArray *rhs[3];
      const char * simulinkBlockPath = ssGetPath(S);
      rhs[ 0 ] = mxCreateString(
        "@coder.connectivity.SimulinkInterface.getSILPILInterface");
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 3, 0.2);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 4, 0.2);
...
{
  {
    ...
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 89, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */
  ...
  {
    {
      ...
      /* dispatch command to the target */
      if (commandDispatch(S, IOBufferPtr, 89)!=XILHOSTAPPSVC_SUCCESS) {
        return;
      }                                /* if */
