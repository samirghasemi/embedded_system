/*
 * Thermostat_SUBSYSTEM_pbs.c
 *
 * Automatically generated s-function with I/O interface for:
 * Component: Thermostat_SUBSYSTEM
 * Component Simulink Path: Thermostat_SUBSYSTEM
 * Simulation Mode: PIL
 *
 */

#define S_FUNCTION_NAME                Thermostat_SUBSYSTEM_pbs
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("p2_2");
      mxArray *rhs[3];
      const char * simulinkBlockPath = ssGetPath(S);
      rhs[ 0 ] = mxCreateString(
        "@coder.connectivity.SimulinkInterface.getSILPILInterface");
...
{
  {
    ...
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 169, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */
  ...
  {
    {
      ...
      /* dispatch command to the target */
      if (commandDispatch(S, IOBufferPtr, 169)!=XILHOSTAPPSVC_SUCCESS) {
        return;
      }                                /* if */
